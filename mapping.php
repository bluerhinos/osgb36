...
function asGridRef($digits = 3) {
 ...
 // get the 100km-grid indices
  ...
  // note use of floor, as ref is bottom-left of relevant square!
	$d = $digits/2;
  $gridRef = sprintf("%s %0{$d}d %0{$d}d",$letPair,$e,$n);
...
function formatlocation($dec, $format,$which, $digits = 0){
	...
	case "DMS":
		...
		///$deg = $zero[3-strlen($deg)].$deg;
		//$min = $zero[2-strlen($min)].$min;
		//$sec = $zero[2-strlen($sec)].$sec;
